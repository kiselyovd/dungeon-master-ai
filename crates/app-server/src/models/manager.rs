...
use crate::models::download::{download_to, DownloadEvent};
...
impl DownloadManager {
    ...
    pub async fn start(&self, id: ModelId) -> Result<(), String> {
        ...
        if !matches!(m.kind, ModelKind::GgufFile) {
            return Err("DiffusersFolder kind requires download_diffusers (Task C.4)".into());
        }
        let url = format!(
