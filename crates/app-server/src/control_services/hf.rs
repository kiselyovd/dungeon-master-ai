//! Concrete HF bridge used by the composition layer.
//!
...
pub(crate) async fn license_check_value(
    state: &AppState,
    repo_id: &str,
) -> Result<serde_json::Value, AppError> {
    let client = make_client(state).await;
    let status = client.check_license(repo_id).await.map_err(hf_err_to_app)?;
    serde_json::to_value(status)
        .map_err(|error| AppError::Internal(format!("serialize license status: {error}")))
}
