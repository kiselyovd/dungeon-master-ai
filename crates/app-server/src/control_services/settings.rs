use app_application::models::settings::{ImagePreset, SettingsConfigV2};
use std::sync::Arc;
...
/// Temporary direct-call compatibility facade for legacy integration tests.
/// Production routing uses `adapter_http::routes::settings::post_settings_v2`.
pub async fn post_settings_v2(
    State(state): State<AppState>,
    Json(config): Json<SettingsConfigV2>,
) -> Result<Json<serde_json::Value>, AppError> {
    validate_settings_v2(&config)?;
    ...
    let result = update.execute(config).await.map_err(|error| match error {
        SettingsUpdateError::Prepare {
            code: "chat_license_restricted",
        } => AppError::BadRequest("openai-compat is blocked by license_restricted_mode".into()),
