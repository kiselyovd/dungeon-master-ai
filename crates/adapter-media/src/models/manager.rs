//! Outbound per-model download state machine.
...
use app_application::ports::secrets::SecretsStore;
...
impl DownloadManager {
    pub fn new(base_dir: PathBuf, secrets: Arc<dyn SecretsStore>) -> Self {
    ...
    pub async fn start(&self, id: ModelId) -> Result<(), String> {
        ...
        let token = self.secrets.get("huggingface_token").await.ok().flatten();
...
mod tests {
    ...
    use app_application::ports::secrets::SecretsError;
    ...
    #[async_trait]
    impl SecretsStore for EmptySecrets {
        async fn get(&self, _key: &str) -> Result<Option<String>, SecretsError> {
            Ok(None)
        }
    ...
    fn test_secrets() -> Arc<dyn SecretsStore> {
