//! Outbound state machine for one sidecar process.
//!
...
use app_application::ports::runtime::{SidecarHandle, SidecarLauncher};
...
mod tests {
    use super::*;
    use app_application::ports::runtime::{SidecarError, SpawnSpec};

    use crate::local_runtime::test_support::MockSidecarLauncher;
    ...
    impl SidecarLauncher for DeadLauncher {
        async fn spawn(&self, _name: &str, _args: &[&str]) -> Result<SidecarHandle, SidecarError> {
            let (_tx, rx) = tokio::sync::mpsc::channel(1);
            Ok(SidecarHandle::from_parts(0, rx, || false, || Ok(())))
        }
    }
